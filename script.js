  ...
  grid.innerHTML = list.length ? list.map(p => {
    ...
    return `<article class="product">
      <div class="product-img">
        ...
        ${productImage(p) ? `<img class="product-photo" src="${productImage(p)}" alt="${String(p.name||'').replace(/"/g,'&quot;')}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"> <div class="product-no-image" style="display:none">Image unavailable</div>` : `<div class="product-no-image">Product image</div>`}
      </div>
...
window.renderCart = function(){
  ...
  box.innerHTML=window.cart.map(item=>{
    ...
    return `<div class="cart-item"><div class="cart-emoji">${productImage(p)?`<img src="${productImage(p)}" alt="">`:'<span>Image</span>'}</div><div><strong>${p.name}</strong><span>${money(p.price)}</span>
...
const SURPRIZZY_UPI_ID = "9819026198-1@okbizaxis"; // Current UPI ID.
const SURPRIZZY_UPI_NAME = ""; // Let the UPI app resolve/display the verified recipient name.
