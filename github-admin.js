...
window.adminLogin=async function(){
  ...
  if(!ghReady()){error.textContent='First enter GitHub Owner, Repository, Branch and Personal Access Token below.';return;}
  try{
    const r=await ghApi(`/repos/${encodeURIComponent(gh.owner)}/${encodeURIComponent(gh.repo)}`); if(!r.ok)throw new Error('GitHub access failed');
    await loadProductsFromGitHub(); localStorage.setItem('surprizzyAdminNumber',number); localStorage.setItem('surprizzyAdminLoggedIn','1');
    document.getElementById('adminLoginOverlay')?.classList.remove('open'); document.getElementById('adminOverlay')?.classList.add('open'); window.renderAdminProducts?.();
  }catch(e){console.error(e);error.textContent='GitHub access failed. Check your repo details and token permissions.'}
};
